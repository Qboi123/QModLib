...
import com.qsoftware.modlib.silentlib.network.internal.DisplayNBTPacket;
...
public class DisplayNBTCommand {
    ...
    private static int runForBlock(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        context.getSource().sendErrorMessage(new TranslationTextComponent("command.qmodlib.nbt.notBlockEntity", title));
        return 0;
    }
    ...
    private static int runForItem(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (stack.isEmpty()) {
            context.getSource().sendErrorMessage(new TranslationTextComponent("command.qmodlib.nbt.noItemInHand"));
            return 0;
        } else if (!stack.hasTag()) {
            context.getSource().sendErrorMessage(new TranslationTextComponent("command.qmodlib.nbt.noItemTag", stack.getDisplayName()));
            return 0;
        }
    ...
    private static void sendPacket(CommandContext<CommandSource> context, CompoundNBT nbt, ITextComponent title) throws CommandSyntaxException {
        ...
        QModLibNetwork.channel.sendTo(msg, netManager, NetworkDirection.PLAY_TO_CLIENT);
    }
