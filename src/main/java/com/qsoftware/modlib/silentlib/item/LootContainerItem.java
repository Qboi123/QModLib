...
package com.qsoftware.modlib.silentlib.item;

import com.qsoftware.modlib.QModLib;
import com.qsoftware.modlib.silentlib.util.LootUtils;
...
public class LootContainerItem extends Item {
    private static final String NBT_ROOT = QModLib.MOD_ID + ".LootContainer";
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add(new TranslationTextComponent("item.qmodlib.lootContainer.table", textTableName).mergeStyle(TextFormatting.BLUE));
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (lootDrops.isEmpty())
            QModLib.LOGGER.warn("LootContainerItem has no drops? {}, table={}", heldItem, getLootTable(heldItem));
    ...
    private static void listItemReceivedInChat(ServerPlayerEntity playerMP, ItemStack stack) {
        ITextComponent itemReceivedText = new TranslationTextComponent(
                "item.qmodlib.lootContainer.itemReceived",
                stack.getCount(),
